...
import static org.jsp.ecommerceapp.util.ApplicationConstants.VERIFY_LINK2;
...
public class ECommerceAppEmailService {
	...
	public String sendWelcomeMailforUser(User user, HttpServletRequest request) {
		String siteUrl = request.getRequestURL().toString();
		String url = siteUrl.replace(request.getServletPath(), "");
		String actual_url = url + VERIFY_LINK2 + user.getToken();
		MimeMessage message = javaMailSender.createMimeMessage();
		MimeMessageHelper helper = new MimeMessageHelper(message);
		try {
